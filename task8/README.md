...
Downstream jobs запускаются асинхронно (`wait: false`), поэтому ожидающая job не удерживает executor Jenkins. Благодаря этому цепочка работает даже при одном доступном executor-е.

После создания jobs необходимо один раз запустить `Job_1` через **Собрать сейчас** или **Build with Parameters**, чтобы Jenkins загрузил Jenkinsfile и зарегистрировал триггер. Для самого первого запуска Jenkinsfile содержит безопасные резервные значения параметров. Последующие запуски выполняются автоматически после изменений в `master`.
